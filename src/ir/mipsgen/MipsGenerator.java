...
public class MipsGenerator implements IRVisitor<Register> {
	...
	private void MipsInit() {
		...
		for (Label funcLabel : IRTreeGenerator.Get().labelSet){
			if (funcLabel.getName().equals(IRTreeGenerator.MAIN_LABEL)){
	...
	private void addMethodDispatchTables() {
		...
		for (Map.Entry<String,Map<String,DispatchAttribute>> dispatchEntry : IRTreeGenerator.Get().dispachMethodsTablesMap.entrySet()){
			...
			for (Map.Entry<String, DispatchAttribute> methodsEntry: dispatchEntry.getValue().entrySet()){
				...
				for (Label funcLabel : IRTreeGenerator.Get().labelSet){
					//main label is in labelSet but should not be in Virtual Tables
					if (!funcLabel.getName().equals(IRTreeGenerator.MAIN_LABEL) && !funcLabel.getName().equals(IRTreeGenerator.PRINT_LABEL)){
						TempLabel tmpFuncLabel = (TempLabel) funcLabel;
	...
	public Register visit(IR_Call call) {
		...
		if (call.label.getName().equals(IRTreeGenerator.PRINT_LABEL)){
			...
			Map<String, DispatchAttribute> classDispatchTable = IRTreeGenerator.Get().dispachMethodsTablesMap.get(callLabel.getClassName());
			//get function Dispatch attribute by name
			DispatchAttribute funcDispatchAttr = classDispatchTable.get(callLabel.getFuncName());
	...
	public Register visit(IR_Function func) {
		...
		if (func.tl._name.equals(IRTreeGenerator.PRINT_LABEL)){
			generatePrintInt();
		}
		else{
